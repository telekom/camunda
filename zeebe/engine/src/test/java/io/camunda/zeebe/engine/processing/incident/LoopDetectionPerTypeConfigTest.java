...
/**
 * Verifies per-{@link BpmnElementType} loop-detection configuration:
 *
 * <ul>
 *   <li>a per-type override is applied instead of the global default, and
 *   <li>a per-type value of {@code 0} disables loop detection for that element type.
 * </ul>
 *
 * <p>Both behaviours are exercised with a single engine configuration: {@code SERVICE_TASK} uses a
 * threshold ({@value #SERVICE_TASK_MAX}) lower than the global default ({@value #GLOBAL_MAX}),
 * while {@code MULTI_INSTANCE_BODY} is disabled.
 */
