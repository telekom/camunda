 ...
 *
 * <p>A multi-instance element is itself a loop: the MI body activates one child per collection
 * item. When the collection size exceeds {@code maxActivations} the activation counter crosses the
 * threshold during the very first (and only) run — no outer process loop is required.
 *
 * <p>The two MI variants trigger the detection differently:
 *
 * <ul>
 *   <li><b>Parallel MI</b>: the body projects {@code collection.size()} child activations upfront.
 *       If that total exceeds {@code maxActivations} an incident fires immediately on the {@code
 *       MULTI_INSTANCE_BODY} — no outer loop and no job completions needed.
 *   <li><b>Sequential MI</b>: the body is excluded from the counter; its children are counted
 *       individually (one increment per child activation). The incident fires on the child
 *       activation that pushes the count beyond {@code maxActivations}.
 * </ul>
 *
 * <h2>Numeric examples ({@code MAX=4})</h2>
 *
 * <pre>
 * Parallel (COLLECTION=5): body activates → projected 5 &gt; MAX=4 → incident immediately
 * Sequential (COLLECTION=5): children 1-4 activate → child 5 would make count=5 &gt; MAX → incident
 * </pre>
 */
